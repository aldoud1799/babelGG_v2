...
# Test 4 — long foreign text is accepted (no app-level length cap)
direct_results = []
direct_monitor = ClipboardMonitor(flash, callback=direct_results.append, min_emit_interval_s=0.0)
long_text = '今日は本当に長いメッセージです。' * 80
direct_monitor._process_clipboard_text(long_text)
assert len(direct_results) == 1, 'Test 4 failed — long foreign text should be processed'
print('PASS  4 — long foreign text accepted')
