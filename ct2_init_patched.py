...
if getattr(_sys, 'frozen', False) and hasattr(_sys, '_MEIPASS'):
    import os as _os, ctypes as _ct, glob as _gl
    _pkg = _os.path.join(_os.path.dirname(_sys.executable), '_internal', 'ctranslate2')
    if _os.path.isdir(_pkg):
        try:
            _os.add_dll_directory(_pkg)
        except Exception:
            pass
        for _d in _gl.glob(_os.path.join(_pkg, '*.dll')):
            try:
                _ct.CDLL(_d)
            except Exception:
                pass
